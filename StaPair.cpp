...
void StaPair::staPair2CC( std::vector <std::string> & DLst, std::vector <std::string> & ELst, std::vector < SacRec > & D_sacV, std::vector < std::string> & FoutnameD,
        ...
        for (int ch2=0; ch2<CHsize2; ch2++)
        {
            ...
            if (doCor(d_sac, sac_amV1[ch1], sac_phV1[ch1], sac_amV2[ch2], sac_phV2[ch2], LagTime, fs, mintlen, tlen, ftlen, frec1, frec2, dist, az, baz))
            {
                #pragma omp critical (PrintCC)
                {
                    ...
                    DLst.push_back(chanL1[ch1]+"_"+chanL2[ch2]+":"+DATE);
                }
            }
...
//                          month<< "." <<day <<" ---- List No. "<<LNo+1<< std::endl;
            else
            {
                #pragma omp critical (PrintCC)
                {
                    ...
                    ss <<month<<"."<<day;
                    std::string DATE(ss.str());
                    ELst.push_back(chanL1[ch1]+"_"+chanL2[ch2]+":"+DATE);
                }
