...
#define PI 3.14159265358979323846
    ...
    float FillGap( float *pbeg, float *pend, float mean1, float mean2, float std, int hlen, int step )
    {
        ...
        *pbeg = rand();
       // std::cout<<"pbeg: "<< pbeg <<" value: "<<*pbeg<<std::endl;
        ...
        for(; p<pend; p+=step)  // bugfix
        {
            ...
            *p = rand() * gdamp;
            slope = ( *p - *(p-step) ) * oostep;
            for(int i=1; i<step; i++) *(p-step+i) = *(p-step) + slope * i;
        }
        p=p-step;
...
void SacRec::Load ()
{
    ...
    /* calculate t0 */
    char koo[9];
    for ( int i=0; i<8; i++ ) koo[i] = shd.ko[i];
    koo[8] = 0;
    float fes;
    int eh, em;
...
inline static int nint( float in )
{
...
void SacRec::cut( float tb, float te, SacRec& sac_result )
{
    ...
    if (nptscpy<0) // 1.05-->1.06
        throw ErrorSR::BadParam( FuncName, "WARNING:: Invalid nptscpy =  " + std::to_string(nptscpy) );
    // copy data
...
void SacRec::merge( SacRec sacrec2 )
{
    ...
    //std::cout<<"begin 1: "<<t1e-t2b<<std::endl;
    ...
    //std::fill(&(sig0[0]), &(sig0[N]), 100000000); // initialize the array to max float
...
int SacRec::arrange(const char* recname, int FileNo)
{
    // count holes
    float maxfloat = std::numeric_limits<float>::max();
    int Nholes=0;
    ...
    if( recname )
    {
        int rec_b[FileNo+1], rec_e[FileNo+1];
        //char recname[200];
        rec_b[0] = 0;
        int j=0;
        for(int i=1; i<shd.npts; i++)
        {
            if(sigsac[i-1] >= maxfloat)
            {
                if(sigsac[i] < maxfloat) rec_b[j] = i;
            }
...
void SacRec::ZoomToEvent( const SAC_HD& eshd, float evlon, float evlat, float tb, float tlen, const std::string ename )
{
    ...
    shd.nzyear = eshd.nzyear;
    shd.nzjday = eshd.nzjday;
    shd.nzhour = eshd.nzhour;
    shd.nzmin = eshd.nzmin;
    shd.nzsec = eshd.nzsec;
    shd.nzmsec = eshd.nzmsec;
    ...
    else   //sps isn't a factor, slower way
    {
        ...
        for(j=0; i<shd.npts-1; j++)
        {
            ...
            if( ti+shd.delta <= tj )
            {
                ti += shd.delta;
                i++;
            }//if(j%1000==0)cerr<<i<<" "<<ti<<j<<" "<<tj<<" "<<endl;}
        }
    }
    sig = std::move(sig2);
...
bool SacRec::CheckPrecNoise()
{
    ...
    if( ndis > lag ) return false;
    nb = int(lag*4/5);
    if( ndis > nb ) nb = ndis;
    for( i = lag+nb; i< 2*lag; i++) noise += pow(sig[i],2);
    noise = sqrt(noise/(lag-nb));

    ndis = (int)floor((dist/4.5-50.)/dt+0.5);
    if( ndis < 10./dt ) return false;
    nb = int(100./dt);
    if(ndis < nb ) nb = ndis;
    for( i = lag-nb; i<= lag+nb; i++) cor_pre += pow(sig[i],2);
    cor_pre = sqrt(cor_pre/(2.*nb));
    if( cor_pre > noise*5)
    {
        std::cout<<"*** WARNING: Large precursor signal. Skipped! ***"<<std::endl;
        return true;
    }
    return false;
}
